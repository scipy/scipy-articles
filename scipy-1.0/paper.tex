...
respectable, with overall project test coverage holding steady at
around 76\% for the last six months (Figure~\ref{fig:coverage}),
but some of the historical components of the code base
would still benefit from increased test coverage. We are still striving
to increase the number of high-quality SciPy usage tutorials beyond
...
# 3 dimesional typed memoryview as an argument
...
\texttt{scipy.integrate} has a new ODE solver interface, and we should aim
...
\texttt{numpy.linalg} unless performance is superior in SciPy,
\texttt{get\_lapack\_funcs} should always use flapack, and
...
\texttt{scipy.fitting}, and relocate \texttt{optimize.curve\_fit}. This 
