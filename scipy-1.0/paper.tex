...
SciPy recently released its first stable version: 1.0.  It signals to
the user community that the library's API has now reached a level of
maturity and stability that warrants trusting it inside research
pipelines.  But the version numbering belies the history of a package
that has already seen extensive adoption in research and industry.

Upon historic introspection, its arrival at this point is surprising,
and somewhat anomalous.  When first started, the library had very
little funding, and was written mainly by graduate students---many of
them without a computer science education, and often without the
...
Despite its success, SciPy still faces challenges: a low ``bus
factor''---the minimum number of contributors that needs to leave to
cause serious difficulty---being perhaps the most serious.
...
For example, Paul Dubois at Lawrence Livermore National Laboratory took over the
...
For example, starting in 2000, the Space Telescope Science Institute (STScI)
ported their entire IDL-based analysis pipeline, IRAF, to Python.
STScI was founded to guide the Hubble Space Telescope including developing
advanced data, archives, and analysis tools for the most famous observatory
in history.
...
In the early
workshops, recurrent topics indicate the state of development, with
...
CiSE 2007 special issue (steering code)
...
SciPy now begins a phase of significant growth and adoption.  The
informal workshops are grown into international conferences with many
...
through a new type of auxhilary package, the scikit.  The tooling,
...
In 2008, the SciPy conference went from being a small gathering of
