...
trlib\cite{doi:10.1080/10556788.2018.1449842} for optimization, % (iterative solving of the trust region problem),
SuperLU\cite{li05,superlu_ug99} for solving sparse linear systems, % (solution of large, sparse, nonsymmetric systems of linear equations),
Qhull\cite{Barber:1996:QAC:235815.235821} for computational geometry, and % (computational geometry)
Cephes\cite{cephes_netlib} for special functions. % (mathematics algorithms).
...
%100,000 annual salary, 112 person-years, 414,473 lines of code -> 11,192,622 (https://www.openhub.net/p/scipy/estimated\_cost, which uses the Basic COCOMO model).

The development cost of SciPy is estimated in excess of 10M dollars by
Open Hub\cite{SciPyValue}.  Yet, the project is largely unfunded, having been
...
used early on to develop documentation.  However, these small amounts of
sporadic funding are not commensurate with the enormous stack of important scientific
software, like those that are used to plan national space missions, that rely
on SciPy.  In the traditional academic funding model, it is easy to support
commercial software, but hard to provide funds for community driven projects,
and as more science depends on open source software, national funding agencies
urgently need to pay attention to this challenge.
Moreover, the scientific Python stack plays an increasingly important role
in industry; however, industry provides little to no funding of community-developed
software, which they depend on.
Industry and funding agencies will have to tolerate the more diverse spending
required to do planning and maintain infrastructure if these open source
projects are to remain a healthy underpinning infrastructure for international
scientific and industrially endeavors.
...
%
%Small funds:
%- meetings (eg Berkeley)
%- GSOC (test infrastructure, recently FFT work, been doing it since, what 2011/2012/2013?)
%- documentation
%
%Problems:
%- funding agencies (NIH, NSF, etc.)
%- foundation have few mechanism for support, while ironically able to fund e.g. Matlab licenses
%- difficulty of hiring full time dev for community projects
%
%Rec:
%- Funders need to address
%- Funding needs to be carefully used to support the community; not just technical work but also process, roadmap, discussing and deciding for new features, etc.
%
%- Many stakeholders, supporting space missions, entire fields, ...
