...
For implementing \emph{new} functionality, Python is the language of choice. If Python 
performance is an issue, then we use Cython, C, C++, or Fortran (listed in order 
of preference). The main motivation for this is maintainability: Cython has the 
highest abstraction level and most Python developers will understand it. C is 
also widely known, and easier to deal with for the current core development team 
than C++ and especially Fortran. While it is not surprising that Python is heavily 
used in SciPy, the usage distribution of the other languages \textemdash despite 
our preferences for implementing new functionality \textemdash warrants discussion. 
