...
unit tests in the test suite, which uses the \texttt{pytest} framework, and coverage of SciPy's approximately $204,000$ lines of code
has held steady at 76.5\% for the last six months (Figure~\ref{fig:coverage}). 
Documentation for the code is automatically built and hosted by
the CircleCI service to facilitate evaluation of documentation changes/integrity.  
Our full test suite also passes with PyPy3, a just-in-time compiled
version of the Python language.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{static/coverage-chart}
\caption{\% coverage of the SciPy codebase by unit tests over the six months
...
In addition to ensuring that unit tests are passing, it is important to ensure that the
the performance of the SciPy codebase improves over time. Since February 2015, the 
performance of the SciPy codebase has been monitored with Airspeed Velocity
(\texttt{asv}\cite{asvref}) library. Figure~\ref{fig:asvbench}
shows example benchmark results for \texttt{scipy.spatial.cKDTree.query} over 
roughly nine years of project history.
...
of toroidal topology to the \texttt{KDTree} (i.e. the \texttt{boxsize} argument 
is $None$).}
\label{fig:asvbench}
\end{figure}
...
is often transferred in some form to the other. At the
codebase level, the \texttt{binned\_statistic} functionality
