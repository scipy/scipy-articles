...
\affil[24]{University of Texas at Austin,
           Instite for Computational Engineering and Sciences,
	   Austin, TX, 78712, USA}
...
When writing algorithms in Cython\cite{behnel2011cython}, it is typically more efficient to call directly into the libraries SciPy wraps rather than indirectly, using SciPy's Python APIs.
SciPy includes Cython wrappers for many BLAS and LAPACK\cite{LAPACK} routines and for many of the special functions provided in the \texttt{scipy.{\allowbreak}special} submodule.
...
These low-level interfaces for Cython can also be used outside of the SciPy codebase to gain direct access to the functions in the wrapped libraries while avoiding the overhead of Python function calls.
This can give performance gains of one or two orders of magnitude for many use cases.

The low-level Cython interfaces can also be used without linking against the wrapped libraries\cite{blas-lapack-wrappers-scipy-2015}.
This lets other extensions avoid the complexity of finding and using the correct libraries.
This is especially important when wrapping libraries written in Fortran since these low-level wrappers can be used without a Fortran compiler and without having to handle all the different Fortran compiler ABI's and name mangling schemes.
