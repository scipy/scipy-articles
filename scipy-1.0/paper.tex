...
over their lifetimes, and the performance of the SciPy codebase was monitored
with \texttt{asv} starting in February of 2015 (PR \#4501). In addition to ensuring that
unit tests are passing, confirming that performance generally
remains constant or improves over the commit hash history of the project allows
us to objectively measure that our codebase is improving. Figure~\ref{fig:asvbench}
shows an example benchmark results for \texttt{scipy.spatial.cKDTree.query} over 
...
of toroidal topology to the \texttt{KDTree} (\texttt{boxsize} argument was ignored). Such results
can be generated using can be run using the command:
