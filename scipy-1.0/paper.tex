...
This model explicitly reject two notions encouraged by the pixel
model: that our signal is best represented as a step-wise functio, and
...
{\tt ndimage} currently follows the pixel model, but we think that the
data point model will serve us better in the long run:

\begin{enumerate}
  \item It makes no assumption about the sampling mechanism used
    (camera, scanner, laser, etc.).
  \item NumPy coordinates will map directly to image coordinates,
