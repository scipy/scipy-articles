...
Future development plans include a major rewrite of the \texttt{scipy.sparse} module, which is already underway\cite{abbasi2018sparse}. This is due to the current interface only supporting two dimensional objects. Even sub-arrays are two dimensional, and dimensions greater than two aren't supported. The new package lacks performance, but it does support dimensions greater than two and a number of other useful features.
